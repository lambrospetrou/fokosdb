...
function pathFor(hashKey: string, ownerAbsDepth: number, depth: number, K: number): number[] {
	return Array.from({ length: depth }, (_, i) => hashChildIndex(kb(hashKey), ownerAbsDepth + i, K));
}
...
describe("HashTopology", () => {
	describe("create basics", () => {
		...
		it("findLeaf returns 0 for any key on a fresh cache", () => {
			...
			expect(cache.findLeaf(kb("any-key"))).toBe(0);
			expect(cache.findLeaf(kb("another-key"))).toBe(0);
		});
	...
	describe("findLeaf on empty cache", () => {
		it("returns 0 regardless of the ownerAbsDepth the cache was created with", () => {
			...
			expect(HashTopology.create(4, 0).findLeaf(kb("key"))).toBe(0);
			expect(HashTopology.create(4, 3).findLeaf(kb("key"))).toBe(0);
			expect(HashTopology.create(4, 10).findLeaf(kb("key"))).toBe(0);
		});
	});
	...
	describe("updateFromHint + findLeaf", () => {
		it("updateFromHint(actualRelDepth=3) → findLeaf returns 3", () => {
			const cache = HashTopology.create(4, 0);
			expect(cache.updateFromHint(kb("key"), 3)).toBe(true);
			expect(cache.findLeaf(kb("key"))).toBe(3);
		});

		it("a different key unaffected by another key's update", () => {
			const cache = HashTopology.create(4, 0);
			cache.updateFromHint(kb("key-a"), 2);
			...
			const depth = cache.findLeaf(kb("key-b"));
			expect(depth).toBeGreaterThanOrEqual(0);
		});

		it("no-op update: already-known depth returns false without modifying findLeaf", () => {
			const cache = HashTopology.create(4, 0);
			cache.updateFromHint(kb("key"), 2);
			expect(cache.updateFromHint(kb("key"), 2)).toBe(false); // nothing new to learn
			expect(cache.updateFromHint(kb("key"), 1)).toBe(false); // shallower than cached
			// findLeaf unchanged
	...
	describe("multiple independent paths", () => {
		it("two keys that hash to different child slots at level 1 don't interfere", () => {
			...
			cache.updateFromHint(kb(keyB), 2);

			expect(cache.findLeaf(kb(keyA))).toBe(3);
			expect(cache.findLeaf(kb(keyB))).toBe(2);
		});
	});
	...
	describe("depth cap", () => {
		...
		it("further updates for the same path don't exceed the cap", () => {
			...
			cache.updateFromHint(kb("key"), 5);
			expect(cache.updateFromHint(kb("key"), 5)).toBe(false);
			expect(cache.findLeaf(kb("key"))).toBe(3);
		});
	});
	...
	describe("budget cap", () => {
		it("stops allocating when budget is exhausted and returns false", () => {
			...
			expect(cache.findLeaf(kb("key"))).toBeLessThanOrEqual(2);
	...
	describe("hybrid: budget kicks in before depth cap", () => {
		it("budget exhausted before maxDepth is reached", () => {
			...
			cache.updateFromHint(kb("key"), 8);
			expect(cache.findLeaf(kb("key"))).toBeLessThan(10);
			expect(cache.findLeaf(kb("key"))).toBeLessThanOrEqual(3);
		});
	});
	...
	describe("staleness self-correction", () => {
		it("updating a path to a greater depth overwrites the shallower cached value", () => {
			...
			cache.updateFromHint(kb("key"), 4);
			expect(cache.findLeaf(kb("key"))).toBe(4);
		});
	});
	...
	describe("toSnapshot / fromSnapshot round-trip", () => {
		it("restores findLeaf results identically", () => {
			const cache = HashTopology.create(4, 0);
			cache.updateFromHint(kb("key-a"), 3);
			cache.updateFromHint(kb("key-b"), 2);

			const snap = cache.toSnapshot();
			const restored = HashTopology.fromSnapshot(snap);

			expect(restored.findLeaf(kb("key-a"))).toBe(cache.findLeaf(kb("key-a")));
			expect(restored.findLeaf(kb("key-b"))).toBe(cache.findLeaf(kb("key-b")));
		});
		...
		it("snapshot includes ownerAbsDepth so routing is correct after restore", () => {
			...
			const restored = HashTopology.fromSnapshot(snap);
			expect(restored.findLeaf(kb("key"))).toBe(cache.findLeaf(kb("key")));
		});
