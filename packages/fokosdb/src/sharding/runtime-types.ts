 ...
 */
export type FokosRangeInput = {
	hashKey: KeyBytes;
	interval: SkInterval;
	descending: boolean;
	/** The resume cursor of the request, when it carries one. The read-through gate validates it against the caller slice. */
...
export type FokosOperation<Req, Res> =
	...
	| {
			...
			local(req: Req, call: FokosLocalCall): Res | Promise<Res>;
			/**
			 * Answers behind the destroy fence. Only a `local` operation can set this: it runs no remote
			 * work, so it cannot add a repartition target after the fence.
			 */
