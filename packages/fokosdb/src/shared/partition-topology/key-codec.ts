...
function encode(key: string | Uint8Array): KeyBytes {
	if (typeof key === "string") {
		if (key.length === 0) {
			throw new FokosValidationError(VALIDATION_CODES.key_encode_empty, { message: "a key must not be an empty string" });
		}
	...
	if (key.byteLength === 0) {
		throw new FokosValidationError(VALIDATION_CODES.key_encode_empty, { message: "a key must not be an empty binary value" });
	}
