...
//
// The suite ran against the routing that `FokosShardingRuntime` replaced and found a defect there: a
// queryItems of a promoted key entered its range tree below the root and answered for one leaf of
// it. `docs/ideas/2026-09-20-query-entry-point-into-a-range-tree.md` holds the symptom, the
// measurement and the reasoning. The runtime routes a range operation by its interval and never by a
// point key, so a query now enters at the root and the suite runs.
