// The range-tree fixture that the queryItems split suites share, and the request arbitrary they
// draw from.
//
// The fixture promotes one hash key into a range tree of several leaves, empties one middle leaf,
// and adds a second hash key on an ordinary partition. Building it costs several hundred writes, so
// a suite builds it once in `beforeAll`.
//
...
import { expectedDataKind, makeTestDB, type ItemData } from "./arbitraries.js";
...
export const COLD_HASH_KEY = "plain:cold";
...
export const label = (sortKey: OptionalQueryKey) => (typeof sortKey === "string" ? sortKey : encodeKey(sortKey).toHex());
/** The payload names its own key, so an item that a page puts in the wrong place fails the comparison. */
export const hotData = (sortKey: OptionalQueryKey): string => `${label(sortKey)}:`.padEnd(ITEM_BYTES, "x");
...
export const put = (db: FokosDB, model: Model, hashKey: QueryKey, sortKey: OptionalQueryKey, data: ItemData) =>
...
/**
 * The request arbitrary of a split suite. `extraBounds` adds positions the fixture cannot know, such
 * as the keys a suite writes after the fixture is built.
 */
 ...
 */
export function budgetFor(choice: number, expected: readonly ModelItem[], fixture: Fixture): { limit?: number; maxResponseBytes?: number } {
