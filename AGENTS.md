...
- A projected point read (`getItem`, each `transactGetItems` item) returns its flat record as `data` in the ordinary item envelope, with `kind: "projected"` and the usual `version` and `ttlAt`. One envelope serves every read: a caller reaches the value through `item.data` and narrows on `kind`, never on the presence of a field. `"projected"` is a public read-result kind only — do not add it to `DataKind`, whose index is the on-disk `data_kind` code. A projected `queryItems` page keeps bare records (`items: ProjectedItem[]`), because its overload types every element exactly.
